...
- ✅ **UI 尺寸優化** - 題目字體優化（比選項大約 1.2 倍），易於閱讀
- ✅ **計時器修正** - 答題倒數計時器正確重置，每次從 30 秒開始
- ✅ **音效流程優化** - 答對/答錯後 3 秒自動播放主選單背景音樂
- ✅ **運動計時器增強** -
  - 彈力帶30下、深蹲15下：60秒計時器（影片靜音+背景音樂）
  - 拳擊有氧60秒：60秒計時器（保留影片聲音，無背景音樂）
  - 瑜珈姿勢：60秒計時器+警告卡片（圖片+提示文字）
...
- **運動懲罰影片展示** (`index.html` - page-videos):
  ...
  - 影片循環播放
  - 計時器功能（智慧型）：
    - 一般影片：按播放鍵→影片靜音+播放倒數背景音樂
    - 拳擊有氧：按播放鍵→保留影片聲音，無背景音樂
    - 瑜珈姿勢：顯示警告卡片「如果60秒沒做完請回去擲骰子位置」
    - 按重置鍵：影片聲音恢復
  - 空白鍵返回主選單
